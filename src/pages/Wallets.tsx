...
export function Wallets() {
  ...
  const [funding, setFunding] = useState<{ [walletId: number]: { amount: string; date: string; note: string } }>({});
  const [showFund, setShowFund] = useState<number | null>(null);

  const fetchCycles = () => {
    wallets.cycles().then(res => {
      setCycles(res.data);
      setLoading(false);
  ...
  const handleFund = async (walletId: number) => {
    const f = funding[cycle.wallet_id] ?? { amount: '', date: new Date().toISOString().split('T')[0], note: '' };
    ...
    setShowFund(null);
    setFunding(prev => ({ ...prev, [walletId]: { amount: '', date: '', note: '' } }));
    fetchCycles();
  };
  ...
  return (
    <div className="page container">
      <div style={{
        ...
        <span style={{ fontSize: 11, color: 'var(--muted)', marginLeft: 10 }}>wallets</span>
      </div>

      <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
        {cycles.map(cycle => {
          const pct = cycle.total_available > 0
            ? (cycle.total_spent / cycle.total_available) * 100
            : 0;
          const barColor = pct > 80 ? 'var(--danger)' : pct > 60 ? 'var(--warning)' : 'var(--accent)';
          ...
          return (
            <div key={cycle.wallet_id} className="card">
              <div style={{
                ...
                <div>
                  <div style={{ fontSize: 13, fontWeight: 500 }}>{cycle.wallet_name}</div>
                  <div style={{
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                  ...
                  <button
                    onClick={() => setShowFund(isFunding ? null : cycle.wallet_id)}
                    style={{
                      background: isFunding ? 'var(--surface)' : 'rgba(76,175,80,0.1)',
              ...
              {isFunding && (
                <div style={{ padding: '12px 16px', borderBottom: '1px solid var(--border)', background: 'var(--surface)' }}>
                  <div className="section-label" style={{ marginBottom: 8 }}>add funding event</div>
                  <div style={{ display: 'flex', gap: 8, marginBottom: 8 }}>
                    <div style={{ flex: 1 }}>
                      ...
                      <input
                        ...
                        value={f.amount}
                        onChange={e => updateFunding(cycle.wallet_id, 'amount', e.target.value)}
                    ...
                    <div style={{ flex: 1 }}>
                      ...
                      <input
                        ...
                        value={f.date}
                        onChange={e => updateFunding(cycle.wallet_id, 'date', e.target.value)}
                  ...
                  <div style={{ marginBottom: 8 }}>
                    ...
                    <input
                      className="input" type="text" placeholder="e.g. dad gave cash"
                      value={f.note}
                      onChange={e => updateFunding(cycle.wallet_id, 'note', e.target.value)}
                      style={{ fontSize: 13 }}
                    />
                  </div>
                  <button
                    ...
                    style={{ height: 40, fontSize: 12, fontFamily: 'var(--font-mono)' }}
                  >
                    confirm funding
                  </button>
                </div>
              ...
              <div style={{ padding: '12px 16px', borderBottom: '1px solid var(--border)' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 8 }}>
                  ...
                  <div style={{ fontFamily: 'var(--font-mono)', fontSize: 10, color: 'var(--muted)' }}>
                    ₹{cycle.total_spent.toLocaleString()} of ₹{cycle.total_available.toLocaleString()}
                  </div>
                </div>
              ...
              <div style={{ padding: '8px 16px' }}>
                {cycle.transactions.slice(0, 4).map((tx, i) => (
                  <div key={tx.id} style={{
                    ...
                    padding: '7px 0',
                    borderBottom: i < Math.min(cycle.transactions.length, 4) - 1
                ...
                {cycle.transactions.length > 4 && (
                  <div style={{ fontSize: 10, color: 'var(--muted)', fontFamily: 'var(--font-mono)', paddingTop: 8 }}>
                    + {cycle.transactions.length - 4} more transactions
                  </div>
                )}
                {cycle.transactions.length === 0 && (
                  <div style={{ fontSize: 11, color: 'var(--muted)', fontFamily: 'var(--font-mono)', padding: '6px 0' }}>
