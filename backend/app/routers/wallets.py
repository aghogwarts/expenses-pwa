...
async def create_wallet(body: WalletCreate, db=Depends(get_db)):
    cursor = await db.execute(
        "INSERT INTO wallets (name, type) VALUES (?, ?)", (body.name, body.type)
    )
    await db.commit()
    row = await (
        await db.execute("SELECT * FROM wallets WHERE id = ?", (cursor.lastrowid,))
    ).fetchone()
    return dict(row)
...
async def get_cycles(db=Depends(get_db)):
    ...
    for wallet in wallets:
        ...
        if prev_funding:
            ...
            leftover = max(
                0, prev_funding["amount"] + leftover - prev_spent_row["total"]
            )
